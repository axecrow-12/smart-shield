...
- **Needs an HTTPS front door.** WebAuthn only runs in a secure context (HTTPS, or `localhost`/`127.0.0.1`) — the plain-LAN-HTTP QR link the standard checkout flow uses will not work here. Run `ngrok http 5050`, then set `PUBLIC_HTTPS_URL=<the ngrok URL>` in `.env` and restart the backend — the Merchant Terminal's "⚡ Vendor Tap" toggle then generates a QR/link pointed straight at that tunnel (`vendorTapLink` in the `create-payment` response, built by `src/utils/network.js::getVendorTapUrl`). Without `PUBLIC_HTTPS_URL` set, the toggle shows a setup message instead of a QR that would fail WebAuthn's secure-context check. See `docs/RUNNING_AND_NAVIGATION.md`.
- Behind a tunnel, `req.protocol` needs `app.set("trust proxy", true)` (set in `src/app.js`) so Express reads the real `https` scheme from `X-Forwarded-Proto` instead of the plain-HTTP connection ngrok forwards internally — without it, WebAuthn's origin check fails with "Unexpected registration response origin".
